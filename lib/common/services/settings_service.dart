...
class SettingsService extends GetxController {
  SettingsService() {
    ...
    enableAutoRotationFullscreen.listen((value) {
      PrefUtil.setBool('enableAutoRotationFullscreen', value);
    });
  ...
  // 自动旋转全屏（iOS/Android移动平台专用）
  // true: 竖屏→横屏时自动进入全屏，横屏→竖屏时自动退出全屏
  // false: 需要手动点击全屏按钮
  final enableAutoRotationFullscreen = (PrefUtil.getBool('enableAutoRotationFullscreen') ?? true).obs;
  ...
  static const List<String> supportSites = [
    ...
    Sites.kuaishouSite,
    Sites.ccSite,
    // Sites.iptvSite,  // 已移除：用户反馈"网络"板块多余
  ];
  ...
  void fromJson(Map<String, dynamic> json) {
    ...
    enableAutoRotationFullscreen.value = json['enableAutoRotationFullscreen'] ?? true;
  ...
  Map<String, dynamic> toJson() {
    ...
    json['enableAutoRotationFullscreen'] = enableAutoRotationFullscreen.value;
