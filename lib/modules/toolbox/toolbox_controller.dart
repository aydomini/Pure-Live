...
class ToolBoxController extends GetxController {
  ...
  void jumpToRoom(String e) async {
    if (e.isEmpty) {
      SmartDialog.showToast(S.current.link_cannot_be_empty);
      return;
    }
    var parseResult = await parse(e);
    if (parseResult.isEmpty || parseResult.first == "") {
      SmartDialog.showToast(S.current.cannot_parse_link);
      return;
    }
  ...
  void getPlayUrl(String e) async {
    ...
    try {
      ...
      var result = await Get.dialog(
        SimpleDialog(
          title: Text(S.current.select_quality),
          children: qualites
              .map(
                (e) => ListTile(
      ...
      await Get.dialog(
        SimpleDialog(
          title: Text(S.current.select_stream),
          children: playUrls
              .map(
                (e) => ListTile(
                  ...
                  onTap: () {
                    ...
                    SmartDialog.showToast(S.current.link_copied);
                  },
                ),
              )
              .toList(),
      ...
      SmartDialog.showToast(S.current.failed_to_get_link);
    } finally {
      SmartDialog.dismiss(status: SmartStatus.loading);
    }
  }
