  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        Padding(padding: const EdgeInsets.symmetric(vertical: 5), child: Text(S.of(context).settings_videofit_title)),
        Padding(
    ...
    return Column(
      ...
      children: [
        ...
        Padding(
          ...
          child: Text(S.of(context).danmaku_display_area, style: Theme.of(context).textTheme.titleSmall),
        ),
        Obx(
          () => Wrap(
            spacing: 8,
            children: [
              ChoiceChip(
                label: Text(S.of(context).danmaku_area_fullscreen),
                selected: controller.danmakuAreaMode.value == 0,
                selectedColor: color,
                labelStyle: TextStyle(
                  color: controller.danmakuAreaMode.value == 0 ? Colors.white : null,
                ),
                onSelected: (selected) {
                  if (selected) {
                    controller.setDanmakuAreaMode(0);
                  }
                },
              ),
              ChoiceChip(
                label: Text(S.of(context).danmaku_area_two_thirds),
                selected: controller.danmakuAreaMode.value == 1,
                selectedColor: color,
                labelStyle: TextStyle(
                  color: controller.danmakuAreaMode.value == 1 ? Colors.white : null,
                ),
                onSelected: (selected) {
                  if (selected) {
                    controller.setDanmakuAreaMode(1);
                  }
                },
              ),
              ChoiceChip(
                label: Text(S.of(context).danmaku_area_one_third),
                selected: controller.danmakuAreaMode.value == 2,
                selectedColor: color,
                labelStyle: TextStyle(
                  color: controller.danmakuAreaMode.value == 2 ? Colors.white : null,
                ),
                onSelected: (selected) {
                  if (selected) {
                    controller.setDanmakuAreaMode(2);
                  }
                },
              ),
            ],
        ...
        Obx(
          () => ListTile(
            ...
            trailing: Text(controller.danmakuFontSize.value.toInt().toString()),
          ),
        ),
        Obx(
          () => ListTile(
            ...
            trailing: Text(controller.danmakuFontBorder.value.toStringAsFixed(2)),
          ),
        ),
      ],
    );
