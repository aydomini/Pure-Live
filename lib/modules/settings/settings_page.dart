...
class SettingsPage extends GetView<SettingsService> {
      ...
      body: ListView(
        ...
        children: <Widget>[
          ...
          if (Platform.isAndroid)
            Obx(
              () => ListTile(
                title: Text(S.of(context).video_player_title),
                subtitle: Text(S.of(context).video_player_subtitle),
                onTap: showVideoSetDialog,
                trailing: Text(controller.videoPlayerIndex.value == 0 ? S.of(context).mpv_player : S.of(context).exo_player),
              ),
            ),
          Obx(
            () => SwitchListTile(
          ...
          ListTile(
            ...
            trailing: Obx(() => Text('${controller.autoRefreshTime}${S.of(context).minutes_unit}')),
            onTap: showAutoRefreshTimeSetDialog,
          ),
          ListTile(title: Text(S.of(context).settings_danmaku_title), onTap: showDanmuSetDialog),
          ListTile(
            title: Text(S.of(context).danmaku_filter_title),
            subtitle: Text(S.of(context).danmaku_filter_subtitle),
            onTap: () => Get.toNamed(RoutePath.kSettingsDanmuShield),
          ),
          ListTile(
            title: Text(S.of(context).platform_settings_title),
            subtitle: Text(S.of(context).platform_settings_subtitle),
          ...
          if (Platform.isAndroid)
            Obx(
              () => SwitchListTile(
                title: Text(S.of(context).compatibility_mode),
                subtitle: Text(S.of(context).compatibility_mode_subtitle),
  ...
  void showDanmuSetDialog() {
    showDialog(
      context: context,
      builder: (context) {
        // 根据屏幕方向调整弹窗宽度：竖屏80%，横屏60%
        final isPortrait = MediaQuery.of(context).orientation == Orientation.portrait;
        final dialogWidth = MediaQuery.of(context).size.width * (isPortrait ? 0.8 : 0.6);
